...
int main(int argc, char *argv[]) {
	...
	differenceImage.save((string(argv[1]) + "_" + string(argv[2]) + "diff.bmp").c_str());

	cout << fixed << setprecision(2) << "Pixels above threshold: " << pixelsAboveThreshold << " (" << (100.0 * pixelsAboveThreshold) / differenceImage.size() << "\%)." << endl;
