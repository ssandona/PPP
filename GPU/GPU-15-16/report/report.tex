...
An image is a 2D structure and for this reason the most convenient choice is to set also a 2D grid. However, setting up a 2D grid, there is not only one way to follow. For this particular project, 4 possible combinations were considered. The first was a square grid of 1D blocks, the second a non square grid with 1D blocks, the third a square grid with 2D blocks and the third a non square grid with 2D blocks.
