...
__global__ void histogram1DKernel(const int width, const int height, const unsigned char *inputImage, unsigned char *grayImage, unsigned int *histogram) {
    ...
    for(i = ((blockIdx.y * gridDim.x + blockIdx.x) * blockDim.x) + threadIdx.x; i < width * height; i += (gridDim.x * blockDim.x) * (gridDim.y * blockDim.y)) {
        ...
        if(cont == 0 || grayPix == last_gray_px) {
            ...
            atomicAdd((unsigned int *)&localHistogram[static_cast< unsigned int >(last_gray_px)], cont);
            cont = 1;
        }
...
int histogram1D(const int width, const int height, const unsigned char *inputImage, unsigned char *grayImage, unsigned int *histogram, int pixelThreads) {
    ...
    unsigned int grid_height = static_cast< unsigned int >(ceil(height / static_cast< float >(pixelThreads)));
    // Execute the kernel
    dim3 gridSize(grid_width, grid_height);
    dim3 blockSize(THREAD_NUMBER);
    cout << "grid size: " << grid_height << "x" << grid_width << " -> threads -> " << (grid_width * grid_height * 256)  << endl;
