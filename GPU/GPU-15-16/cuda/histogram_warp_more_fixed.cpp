...
extern int histogram1D(const int width, const int height, const unsigned char * inputImage, unsigned char * grayImage, unsigned int * histogram, int grid_height, grid_width);
...
int main(int argc, char *argv[]) {
	...
	// Save output
	grayImage.save(("./" + string(argv[1]) + ".gray.uni.par.bmp").c_str());
	histogramImage.save(("./" + string(argv[1]) + ".hist.warp.more.fixed.par.bmp").c_str());
