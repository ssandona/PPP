...
int histogram1D(const int width, const int height, const unsigned char *inputImage, unsigned char *grayImage, unsigned int *histogram, int pixelThreads) {
    ...
    unsigned int grid_width = static_cast< unsigned int >(ceil(height / (float)B_WIDTH));

    unsigned int grid_height = static_cast< unsigned int >(ceil(ceil(height / (float)B_HEIGHT) / (float)pixelThreads));
    // Execute the kernel
