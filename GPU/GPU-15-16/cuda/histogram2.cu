...
__global__ void histogram1DKernel(const int width, const int height, const unsigned char *inputImage, unsigned char *grayImage, unsigned int *histogram) {
    ...
    __shared__ unsigned char localImagePortion[B_WIDTH*B_HEIGHT*3*sizeof(unsigned char)];
    ...
    atomicAdd((unsigned int *)&histogram[static_cast< unsigned int >(globalIdx)], localHistogram[globalIdx]);
