...
extern int histogram1D(const int width, const int height, const unsigned char * inputImage, unsigned char * grayImage, unsigned int * histogram, int grid_size);
...
int main(int argc, char *argv[]) {
	...
	int r=histogram1D(inputImage.width(), inputImage.height(), inputImage.data(), grayImage.data(), histogram, grid_size);
	if(r==1){
		cout << "ERROR\n";
		return 1;
	}
	...
	// Save output
	grayImage.save(("./" + string(argv[1]) + ".gray.fixed.square.more.par.bmp").c_str());
	histogramImage.save(("./" + string(argv[1]) + ".hist.fixed.square.more.par.bmp").c_str());
