...
__global__ void histogram1DKernel(const int width, const int height, const unsigned char *inputImage, unsigned char *grayImage, unsigned int *histogram) {
    ...
    unsigned int inBlockIdx = blockDim.x * threadIdx.y + threadIdx.x ;
    ...
    for(k = (blockDim.x * gridDim.x * i)+j; k < width * height; k += (gridDim.x * blockDim.x) * (gridDim.y * blockDim.y)) {
        float grayPix = 0.0f;
        //if(blockIdx.x >= 10) {
...
int histogram1D(const int width, const int height, const unsigned char *inputImage, unsigned char *grayImage, unsigned int *histogram, int grid_size) {
