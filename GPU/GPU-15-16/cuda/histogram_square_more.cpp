...
extern int histogram1D(const int width, const int height, const unsigned char * inputImage, unsigned char * grayImage, unsigned int * histogram, int pixelThreads);
...
int main(int argc, char *argv[]) {
	...
	int r=histogram1D(inputImage.width(), inputImage.height(), inputImage.data(), grayImage.data(), histogram, atoi(arv[2]));
	if(r==1){
		cout << "ERROR\n";
		return 1;
	}
	...
	// Save output
	grayImage.save(("./" + string(argv[1]) + ".gray.square.more.par.bmp").c_str());
	histogramImage.save(("./" + string(argv[1]) + ".hist.square.more.par.bmp").c_str());
